...
# Header for autogenerated files
HEADER_TEMPLATE = (
    "# --------------------------------------------------------------------------\n"
...
def generate_containerfile(config, repo_path):
    ...
    if template_path.exists():
        ...
        content = HEADER_TEMPLATE.format(source="Containerfile.j2") + template.render(config)
    ...
    if pkg_template_path.exists():
        ...
        content = HEADER_TEMPLATE.format(source="Containerfile.pkg.j2") + template.render(config)
...
def main():
    ...
    for repo in REPO_ROOT.glob("daemonless/*"):
        ...
        if config.get('skip_readme'):
            ...
            # Note: READMEs are usually generated from README.j2 which is in templates dir, not repo dir
            # But the HEADER logic implies we want to mark it as autogenerated.
            # README.md is Markdown, so # comments are headers. HTML comments <!-- --> are safer for hiding metadata.
            # However, standard practice for autogenerated files often uses # for comments if possible or just prepended text.
            # For Markdown, <!-- ... --> is invisible.
            ...
            readme_content = readme_header + template.render(config, render_mode="github")
